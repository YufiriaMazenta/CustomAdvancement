...
public class V1_17_R1AdvancementWrapper extends AbstractAdvancementWrapper {

    public V1_17_R1AdvancementWrapper(String key, JsonObject json) {
        super(key, json);
    }

    public V1_17_R1AdvancementWrapper(String key) {
        super(key);
    }
    ...
    public boolean grant(Player player) {
        ...
        Advancement advancement = ((CraftServer) Bukkit.getServer()).getServer().getAdvancementData().a(new MinecraftKey(super.getKey()));
        if (advancement == null) {
            throw new NullPointerException("Advancement " + super.getKey() + " is null");
        }
        AdvancementProgress advancementProgress = advancementDataPlayer.getProgress(advancement);
        if (!advancementProgress.isDone()) {
            for (String criterion : advancementProgress.getRemainingCriteria()) {
                advancementDataPlayer.grantCriteria(advancement, criterion);
            }
        }
        return true;
    }
    ...
    public boolean revoke(Player player) {
        ...
        if (!advancementProgress.b()) {
            for (String criterion : advancementProgress.getAwardedCriteria()) {
                advancementDataPlayer.revokeCritera(advancement, criterion);
            }
        }
        return true;
    }
