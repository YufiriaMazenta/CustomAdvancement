...
import com.github.yufiriamazenta.customadv.adv.V1_19_R2AdvancementWrapper;
...
public enum V1_19_R2AdvancementManager implements IAdvancementManager {

    INSTANCE;

    @Override
    public AbstractAdvancementWrapper advancementWrapper(String key, JsonObject jsonObject) {
        return new V1_19_R2AdvancementWrapper(key, jsonObject);
    }

    @Override
    public AbstractAdvancementWrapper advancementWrapper(String key) {
        return new V1_19_R2AdvancementWrapper(key);
    }
    ...
    public void reloadPlayerAdvancements() {
        MinecraftServer minecraftServer = ((CraftServer) Bukkit.getServer()).getServer();
        PlayerList playerList = minecraftServer.ab();
        AdvancementDataWorld advancementDataWorld = minecraftServer.ay();
        for (EntityPlayer player : playerList.k) {
            AdvancementDataPlayer advancementDataPlayer = player.N();
